function plotMovementCDFAcrossDays(animalName,subset)
...
%     subplot(1,size(subset,1),iExpt);
    workingList = getExperimentsByAnimalAndDate(animalName,subset{iExpt},'spon');
...
workingList = getExperimentsByAnimalAndDate(animalName,subset{1},'spon'); % just grab the first expt from list
workingList = workingList(1,:);
