...
%can we use parallel computing to run a command to setup (and wait for)
...
S = varargin{1};
if ~isfield(S,'exptIndexLast')
    return % if we haven't already run an experiment today.
end
%1. identify last expt? S.exptIndexLast
%2. copy files (including video)?
%3. import named expt?

[date,index] = fixDateIndexToFiveForSynapse(S.exptDate,S.exptIndexLast);
dirStrRecSource = ['\\' S.recordingComputer '\e\Data\Synapse\' date '-' index '\'];
