...
for iExpt = 1:length(exptIDs)
    ...
    if animalNum > 51 % New system videos started with EEG52
        vidFileName = [getPathGlobal('W') 'PassiveEphys\20' date(1:2) '\' date '-' index '\20' date(1:2) '_' date '-' index '_Cam1.avi'];
    ...
    try
        ...
        movtFile = [getPathGlobal('M') 'PassiveEphys\20' date(1:2) '\' exptname '\' exptname '-movementBinary.mat'];
        dirCheck = dir(movtFile);
        
        if ~isempty(dirCheck)
            % if there is a saved movement file, load it, and use the previous ROI
            disp('using saved ROI');
            prevAnalysis = load([getPathGlobal('M') 'PassiveEphys\20' date(1:2) '\' exptname '\' exptname '-movementBinary.mat'],'fullROI');
