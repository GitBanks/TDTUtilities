...
uicontrol('style','text',...
    'units','pix',...
    'position',[10 550 150 40],...
    'fontweight','bold',...
    'string','list of expts');
uicontrol('style','text',...
    'units','pix',...
    'position',[100 665 80 30],...
    'string','stim/resp Peaks');
uicontrol('style','text',...
    'units','pix',...
    'position',[180 665 80 30],...
    'string','stim/resp Peaks');
uicontrol('style','text',...
    'units','pix',...
    'position',[260 665 80 30],...
    'string','stim/resp Peaks');
uicontrol('style','text',...
    'units','pix',...
    'position',[340 665 80 30],...
    'string','Bandpower');
uicontrol('style','text',...
    'units','pix',...
    'position',[420 665 80 30],...
    'string','Move magni');
uicontrol('style','text',...
    'units','pix',...
    'position',[500 665 80 30],...
    'string','Move eCDF');
S = listOfDates(S);
uiwait(S.fh);
close(dbConn);
...
S.pf = uicontrol('style','push',...
    'units','pix',...
    'posit',[420 650 80 30],... % we want to overwrite last button (to avoid repeating/changing a list) was% 'posit',[405 650 120 30],
    'string', 'Plot Day',...
    'fontsize',9,...
    'callback',{@stimAndMovementPlot,S});
S.pf = uicontrol('style','push',...
    'units','pix',...
    'posit',[500 650 80 30],... % we want to overwrite last button (to avoid repeating/changing a list) was% 'posit',[405 650 120 30],
    'string', 'Plot Range',...
    'fontsize',9,...
    'callback',{@movementCDFPlotAcrossDays,S});
...
function [S] = stimAndMovementPlot(varargin)
[S] = varargin{3};
S.userDateSelection = get(S.ls,'Value');
%workingList = getExperimentsByAnimalAndDate(S.animalName,S.justDates{S.userDateSelection},'stim/resp');
workingList = getExperimentsByAnimalAndDate(S.animalName,S.justDates{S.userDateSelection});
for iList = 1:size(workingList,1)
    plotStimAndMovement(workingList{iList,1}(1:5),workingList{iList,1}(7:9));
end

function [S] = movementCDFPlotAcrossDays(varargin)
[S] = varargin{3};
S.userDateSelection = get(S.ls,'Value');
subset = S.justDates(S.userDateSelection);
plotMovementCDFAcrossDays(S.animalName,subset);
