...
if ~exist('gMouseEphys_out','var') || ~exist('gBatchParams','var')
    error('need ephys data and batch params!');
end
...
if sum(contains(fieldnombres,'delta')) > 0 %assume wpli structure has band names listed
...
%                 meanMovementPerWindow(excludeMovementTrials) = nan;
    ...
    try
        if contains(fieldnames(gMouseEphys_out.(animalName).(thisDate).(thisExpt)),'trialsKept')
