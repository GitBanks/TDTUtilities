...
clear all
close all
exptTable = readtable('M:\Zarmeen\Data\SR Model Fits\SRFilt.csv');

nExpt = size(exptTable,1);

for iList = 1:nExpt
    exptDate = exptTable.Index(iList);
    exptDateChar = char(exptDate);
    exptDate = exptDateChar(1:5);
    exptIndex = exptDateChar(7:9);
    animal = char(exptTable.Animal(iList));
    outPath2 = [getPathGlobal('M') 'PassiveEphys\20' char(exptDate(1:2)) '\' char(exptTable.Index(iList)) '\'];
if ~exist(outPath2,'dir')
    mkdir(outPath2);
end
    load([outPath2 char(exptTable.Index(iList)) '_singleTrialPeakDataFilt'],'singleTrialPeakDataFilt')
    data(iList).Animal = exptTable.Animal(iList);
    data(iList).dateIndex = exptTable.Index(iList);
    data(iList).Drug = exptTable.Treatment{iList};
    data(iList).StimIntensity =  singleTrialPeakDataFilt.stimArrayNumeric;
    data(iList).TrialData = singleTrialPeakDataFilt.pkVals.data;
end

%You have to manually go through the recordings now
iList = 26;
x = size(data(iList).TrialData ,2)
...
descrip = char(exptTable.Index(iList));
outPath = ['M:\Zarmeen\Data\SR Model Fits\SingleTrialResponsesFilt'];
